...
class refills_model extends MY_Model {
	public $tb_users;
	public $tb_order;
	public $tb_refills;
	...
	function get_refills_logs_list($total_rows = false, $status = "", $limit = "", $start = ""){
		$data  = array();
		if (get_role("user")) {
			$this->db->where("r.refill_client_id", session("uid"));
		}
		if ($limit != "" && $start >= 0) {
			$this->db->limit($limit, $start);
		}
		$this->db->select('r.*, o.*, u.email as user_email, s.name as service_name, api.name as api_name');
		$this->db->from($this->tb_refills." r");
		$this->db->join($this->tb_order." o", "o.id = r.refill_order_id", 'left');
		$this->db->join($this->tb_users." u", "u.id = r.refill_client_id", 'left');
		$this->db->join($this->tb_services." s", "s.id = r.refill_service_id", 'left');
		$this->db->join($this->tb_api_providers." api", "api.id = r.refill_api_provider_id", 'left');
		if($status != "all" && !empty($status)){
			$this->db->where("r.refill_status", $status);
		}
		$this->db->order_by("r.refill_id", 'DESC');
	...
	function get_count_refills($status = ""){
		if (get_role("user")) {
			$this->db->where("refill_client_id", session("uid"));
		}
		$this->db->select("refill_id");
		$this->db->from($this->tb_refills);
		if($status != "all" && !empty($status)){
			$this->db->where("refill_status", $status);
		}
		$query = $this->db->get();
