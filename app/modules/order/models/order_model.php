...
class order_model extends MY_Model {
	...
	function get_order_logs_list($total_rows = false, $status = "", $limit = "", $start = ""){
		...
		$this->db->select('o.*, u.email as user_email, s.name as service_name, s.refill as service_refill, api.name as api_name');
	...
	function get_is_already_refill($order_id){
		$where = "(`refill_status` = 'awaiting' or `refill_status` = 'processing' or `refill_status` = 'inprogress'  or `refill_status` = 'pending' or `refill_status` = '') AND refill_order_id = '".$order_id."' AND refill_client_id = '".session("uid")."' ";
		$data  = array();
	...
	function get_refill_order_details($id){
		$data  = array();
		if (get_role("user")) {
			$this->db->where("o.uid", session("uid"));
		}
