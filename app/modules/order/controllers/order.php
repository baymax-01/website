...
class order extends MX_Controller {
	...
	public function refill_order($id){

		$refill_data = $this->model->get_refill_order_details($id);

		if(!empty($refill_data)){

			if($this->model->get_is_already_refill($id) != "0"){ 
				ms(array(
					"status"  => "error",
			...
			if(isset($response->refill) && $response->refill != "") {
				...
				$data_refill = array(
					...
					'refill_api_order_id'      => $response->refill,
				...
				if($this->db->insert($this->tb_refills, $data_refill)){
					ms(array(
						"status"  => "success",
						"message" => lang("Refill_successfull")
					));
				}else{
					ms(array(
						"status"  => "error",
						"message" => "Refill could not be completed"
					));
				}
			}	
	...
	// Delete
	function ajax_log_delete_item($ids = ""){
