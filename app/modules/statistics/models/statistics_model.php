...
class statistics_model extends MY_Model {
	...
	public function get_data_logs(){
		...
		$data_orders_chart_spline = array(
			"time" 				=> json_encode($this->stats_log($this->tb_orders, 'completed')->date),
			"completed"  		=> json_encode($this->stats_log($this->tb_orders, 'completed')->value),
			"processing"   		=> json_encode($this->stats_log($this->tb_orders, 'processing')->value),
			"pending"    		=> json_encode($this->stats_log($this->tb_orders, 'pending')->value),
			"in progress"    	=> json_encode($this->stats_log($this->tb_orders, 'inprogress')->value),
			"partial"    		=> json_encode($this->stats_log($this->tb_orders, 'partial')->value),
			"canceled"    		=> json_encode($this->stats_log($this->tb_orders, 'canceled')->value),
			"refunded"    		=> json_encode($this->stats_log($this->tb_orders, 'refunded')->value),
		);
